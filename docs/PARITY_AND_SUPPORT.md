...
| RAW | Camera RAW decoding and processing | Implemented in pre-alpha interface and core, workflow limited | `CIRAWFilter` is authority for real RAW decoding; common-image fallback and empty legacy RAW-pin recovery exist; DNG 1.7.1 metadata inspection and fixed-size ASCII tag patching preserve raw payloads; Bayer/X-Trans decoding, LibRaw fallback, complete DNG encoding, supported-camera list, and licensed real RAW fixture test remain incomplete |
| Local edits | Masks, healing, red-eye, geometry | Partially implemented in pre-alpha core, workflow not yet integrated | Versioned mask primitives, mask graphs, authoring, duplicate/synchronise requests, stale-result gating, deterministic brush/linear/radial/colour/luminance/depth rasterisation, graph composition, masked exposure/colour grading, crop, rotation, baseline transform rendering, and bounded deterministic clone/healing recipes are tested; AI-generated masks, real depth data, texture-aware healing, red-eye, and complete mask UI remain incomplete |
| AI features | Denoise, selections, generative functions | Partially implemented in pre-alpha core, workflow not yet integrated | Offline Apple Vision foreground, subject, and person masks now produce durable grayscale coverage with asset/mask/revision/request identity and stale-result rejection; sky, object, background, landscape, real depth, denoise, super-resolution, generative output, packaged proprietary weights, and end-user AI controls remain incomplete |
| Metadata | IPTC, EXIF, XMP, keywords, face data | Partially implemented in pre-alpha interface and core, workflow limited | Durable IPTC/EXIF/XMP-like fields, keyword normalisation/search, v1-to-v2 catalog migration, preset save/list/apply/delete, reopen, source-immutability tests, and a Library metadata editor exist; batch metadata UI, embedded/sidecar XMP writing, full field coverage, hierarchical keyword taxonomies, and face data remain incomplete |
