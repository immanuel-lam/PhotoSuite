...
| Metadata | IPTC, EXIF, XMP, keywords, face data | Partially implemented in pre-alpha interface and core, workflow limited | Durable IPTC/EXIF/XMP-like fields, atomic XMP sidecar write/read, bounded standards-based Adobe XMP Develop-preset import with skipped-field reporting, keyword normalisation/search, hierarchical keyword nodes and asset assignments, v1-to-v6 catalog migration, preset save/list/apply/delete, reopen, source-immutability tests, and a Library metadata editor exist ([keyword tests](../Tests/CatalogCoreTests/KeywordAndVirtualCopyTests.swift), [XMP tests](../Tests/PhotoWorkflowTests/XMPSidecarWriterTests.swift), [preset-import tests](../Tests/PhotoWorkflowTests/XMPDevelopPresetImporterTests.swift)); batch metadata UI, embedded XMP writing, full field coverage, complete taxonomy authoring, and face data remain incomplete |
