...
| --- | --- | --- | --- |
| Import | Photo import and source management | Implemented in pre-alpha interface and core, workflow limited | Native local multi-file import, import-review selection, Add/Copy/Move transfer controls, SHA-256 plus byte-count duplicate detection, atomic verified transfers, bookmarks, and item-error handling exist ([import tests](../Tests/PhotoWorkflowTests/ImportManagementTests.swift)); broad catalog duplicate indexing and release support remain incomplete |
| Library | Catalog, folders, collections, search, metadata | Implemented in pre-alpha interface and core, workflow limited | SQLite catalog/search/jobs/bookmarks/backups and native browsing, one-asset and folder-based fingerprint-validated missing-source relink, rating, colour-label, search, smart-filter, durable regular/smart collections, photo-stack controls, hierarchical keyword nodes and asset assignments, folder hierarchy and asset assignment, native keyword/folder controls, independent virtual-copy recipe history, native virtual-copy controls, durable face-region/label records, local Vision face-rectangle detection, and native People annotation review/label controls exist ([catalog tests](../Tests/CatalogCoreTests/KeywordAndVirtualCopyTests.swift), [folder tests](../Tests/CatalogCoreTests/FolderCatalogTests.swift), [face tests](../Tests/CatalogCoreTests/FaceCatalogTests.swift), [relink tests](../Tests/PhotoWorkflowTests/MissingSourceRelinkTests.swift), [batch relink tests](../Tests/PhotoWorkflowTests/BatchMissingSourceRelinkTests.swift), [Vision face tests](../Tests/RenderCoreTests/VisionFaceDetectionTests.swift), [workspace face tests](../Tests/PhotoWorkflowTests/PhotoWorkspaceFaceTests.swift)); automatic background folder scans, identity recognition, keyword-node authoring, Lightroom migration, and broader metadata workflows remain incomplete |
| Develop | Non-destructive adjustment workflow | Implemented in pre-alpha interface and core, workflow limited | Native basic adjustment, crop canvas gestures, rotation, undo, redo, and before/after controls exist; durable recipes, Develop preset save/list/apply/delete, and tested baseline tone, white-balance, transform, detail, bounded lens distortion, chromatic-aberration approximation, defringe, vignette correction, creative vignette, grain, dehaze, calibration, black-and-white, HDR-intent, three-way grade, and histogram core exist ([crop tests](../PhotoSuiteApp/Tests/CropInspectorTests.swift), [crop gesture tests](../PhotoSuiteApp/Tests/CropCanvasGestureTests.swift), [Develop render tests](../Tests/RenderCoreTests/DevelopRenderTests.swift), [optics UI tests](../PhotoSuiteApp/Tests/ModernUIContractTests.swift)); an attribution-preserving Lensfun XML importer and bounded profile mapping are also available ([Lensfun tests](../Tests/PhotoDomainTests/LensProfileTests.swift)); Adobe-equivalent camera profiles, full Lensfun polynomial evaluation, full HDR output, and complete workflow validation remain incomplete |
...
| Local edits | Masks, healing, red-eye, geometry | Implemented in pre-alpha interface and core, workflow limited | Versioned mask primitives, native mask authoring controls, mask graphs, duplicate/synchronise requests, stale-result gating, deterministic brush/linear/radial/colour/luminance/depth rasterisation, graph composition, masked exposure/colour grading, crop, rotation, baseline transform rendering, bounded deterministic clone/healing recipes, user-specified manual red-eye correction, and a validated native retouch inspector with durable clear/reopen coverage are tested ([mask UI tests](../PhotoSuiteApp/Tests/MaskAuthoringInspectorTests.swift), [retouch UI tests](../PhotoSuiteApp/Tests/ModernUIContractTests.swift), [retouch workflow tests](../Tests/PhotoWorkflowTests/PhotoWorkspaceTests.swift), [retouch render tests](../Tests/RenderCoreTests/RetouchRenderTests.swift)); smart selections, real depth data, automatic eye detection, texture-aware healing, and full Lightroom-equivalent mask behaviour remain incomplete |
| AI features | Denoise, selections, generative functions | Partially implemented in pre-alpha interface and core, workflow limited | Offline Apple Vision foreground, subject, and person masks now produce durable grayscale coverage from the Develop mask authoring inspector with asset/mask/revision/request identity and stale-result rejection ([Vision mask tests](../Tests/RenderCoreTests/VisionAIMaskServiceTests.swift)); optional local Core ML packs have manifest, licence/provenance, checksum, memory, speed, and unavailable-status contracts ([model-pack tests](../Tests/PhotoDomainTests/AIModelPackRegistryTests.swift)); sky, object, background, landscape, and depth now have explicit typed unavailable/optional-model capability results and stale-result tests ([semantic capability tests](../Tests/RenderCoreTests/VisionSemanticMaskCapabilitiesTests.swift)), but no default weights or inference implementation are bundled; denoise, super-resolution, generative output, packaged proprietary weights, and remaining AI selections remain incomplete |
| Metadata | IPTC, EXIF, XMP, keywords, face data | Partially implemented in pre-alpha interface and core, workflow limited | Durable IPTC/EXIF/XMP-like fields, atomic XMP sidecar write/read, native metadata-inspector sidecar actions with security-scope rejection, bounded standards-based Adobe XMP Develop-preset import with skipped-field reporting, keyword normalisation/search, hierarchical keyword nodes and asset assignments, schema-v1-to-v7 catalog migration, preset save/list/apply/delete, durable face regions/labels/search/delete, local Vision face-rectangle detection, native People annotation review/label controls, reopen, source-immutability tests, and a Library metadata editor exist ([keyword tests](../Tests/CatalogCoreTests/KeywordAndVirtualCopyTests.swift), [face tests](../Tests/CatalogCoreTests/FaceCatalogTests.swift), [Vision face tests](../Tests/RenderCoreTests/VisionFaceDetectionTests.swift), [workspace face tests](../Tests/PhotoWorkflowTests/PhotoWorkspaceFaceTests.swift), [XMP tests](../Tests/PhotoWorkflowTests/XMPSidecarWriterTests.swift), [sidecar service tests](../Tests/PhotoWorkflowTests/MetadataSidecarServiceTests.swift), [preset-import tests](../Tests/PhotoWorkflowTests/XMPDevelopPresetImporterTests.swift)); batch metadata UI, embedded XMP writing, full field coverage, complete taxonomy authoring, and identity recognition remain incomplete |
...
| Tethering | Camera tethered capture | Partially implemented in pre-alpha interface and core, workflow limited | ImageCaptureCore discovery, status/events, capture-request plumbing, native status controls, import review, and watched-folder import with settling, duplicate detection, atomic copy, cancellation, and recovery are tested ([capture tests](../Tests/PhotoWorkflowTests/CapturePipelineTests.swift), [review tests](../Tests/PhotoWorkflowTests/LibraryImportReviewTests.swift)); physical camera transfer and Canon/Nikon/Sony/Fujifilm/Leica vendor adapters remain unverified or external |
...
| Location, face, and other sensitive metadata | Partially implemented in pre-alpha interface and core, workflow limited | Deliver has selected JPEG metadata policies; GPS extraction, durable face regions/labels, unlabeled local Vision face geometry, and a People annotation inspector for optional user labels exist, while EXIF, IPTC, XMP, identity recognition, location round-trip, and default-disclosure rules remain incomplete |
...
| Image import | Implemented in pre-alpha interface and core, workflow limited | Local multi-file import, native review selection with Add/Copy/Move, fingerprint duplicate detection, verified file transfer, one-asset relink, and folder-based batch relink exist; background folder watching and production workflow support remain incomplete |
...
| Catalog migration and recovery | Implemented in pre-alpha interface and core, workflow limited | SQLite migrations, integrity checks, bookmarks, backups, jobs, reopen tests, and native one-asset plus folder-based missing-source relink flows with fingerprint and security-scope checks exist; automatic background scans and full user recovery tooling remain incomplete |
| Editing, preview, and export | Implemented in pre-alpha interface and core, workflow limited | Library, Develop, Deliver, and Professional Workspace screens exist with bounded previews, histogram, crop-canvas gestures, baseline Develop groups, bounded optics/effects, durable Develop presets, colour grade, native mask authoring, deterministic local mask rendering, verified Vision masks, deterministic manual red-eye rendering, durable metadata and People annotation editing, native XMP sidecar actions, folder/keyword/virtual-copy catalog records, native MapKit/print/device surfaces, PDF/slideshow/static-gallery output, and atomic multi-format delivery; broader AI execution, complete metadata UI, broader codecs, and production workflow verification remain incomplete |
