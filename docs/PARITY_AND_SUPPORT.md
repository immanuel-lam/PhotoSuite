...
| Export | JPEG and other export workflows | Implemented in pre-alpha interface and core, workflow limited | Deliver provides atomic sRGB JPEG, PNG, HEIF, and TIFF export with tested resize, metadata, text-watermark, output-sharpening, cancellation, duplicate-destination checks, and sequential batch publication; publish services, broader codec coverage, and release validation remain incomplete |
