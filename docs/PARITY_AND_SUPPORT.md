...
| Library | Catalog, folders, collections, search, metadata | Implemented in pre-alpha interface and core, workflow limited | SQLite catalog/search/jobs/bookmarks/backups and native browsing, rating, colour-label, search, smart-filter, durable regular/smart collections, photo-stack controls, hierarchical keyword nodes and asset assignments, native keyword controls, independent virtual-copy recipe history, and native virtual-copy controls exist ([catalog tests](../Tests/CatalogCoreTests/KeywordAndVirtualCopyTests.swift), [workspace tests](../Tests/PhotoWorkflowTests/PhotoWorkspaceTests.swift)); keyword-node authoring, folder hierarchy, faces, Lightroom migration, and broader metadata workflows remain incomplete |
...
| AI features | Denoise, selections, generative functions | Partially implemented in pre-alpha core, workflow not yet integrated | Offline Apple Vision foreground, subject, and person masks now produce durable grayscale coverage with asset/mask/revision/request identity and stale-result rejection ([Vision mask tests](../Tests/RenderCoreTests/VisionAIMaskServiceTests.swift)); sky, object, background, landscape, real depth, denoise, super-resolution, generative output, packaged proprietary weights, and end-user AI controls remain incomplete |
| Metadata | IPTC, EXIF, XMP, keywords, face data | Partially implemented in pre-alpha interface and core, workflow limited | Durable IPTC/EXIF/XMP-like fields, keyword normalisation/search, hierarchical keyword nodes and asset assignments, v1-to-v4 catalog migration, preset save/list/apply/delete, reopen, source-immutability tests, and a Library metadata editor exist ([keyword tests](../Tests/CatalogCoreTests/KeywordAndVirtualCopyTests.swift)); batch metadata UI, embedded/sidecar XMP writing, full field coverage, complete taxonomy authoring, and face data remain incomplete |
